...
class DatabricksStack(core.Stack):
    ...
    def __init__(self, scope: core.Construct, **kwargs) -> None:
        ...
        cross_account_policy = iam.Policy(
            self,
            id=f"iam-{self.deploy_env}-databricks-cross-account-policy",
            policy_name=f"iam-{self.deploy_env}-databricks-cross-account-policy",
            statements=[
                iam.PolicyStatement(
                    actions=[
        ...
        bucket = s3.Bucket(
            self,
            id=f"s3-{self.deploy_env}-dados-databricks-bucket",
            bucket_name=f"s3-{self.deploy_env}-dados-databricks-bucket",
        )
        bucket.add_to_resource_policy(
            iam.PolicyStatement(
                principals=[iam.AccountPrincipal(account_id="226904475050")],
