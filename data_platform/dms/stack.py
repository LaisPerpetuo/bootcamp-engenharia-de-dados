    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.rds_endpoint = dms.CfnEndpoint(
            scope,
            f"dms-{self.common_stack.deploy_env}-orders-rds-endpoint",
            endpoint_type="source",
            endpoint_identifier=f"dms-source-{self.common_stack.deploy_env}-orders-rds-endpoint",
        ...
        self.s3_endpoint = dms.CfnEndpoint(
            scope,
            f"dms-{self.common_stack.deploy_env}-orders-s3-endpoint",
            endpoint_type="target",
            engine_name="s3",
            endpoint_identifier=f"dms-target-{self.common_stack.deploy_env}-orders-s3-endpoint",
        ...
        self.dms_sg = ec2.SecurityGroup(
            scope,
            f"dms-{self.common_stack.deploy_env}-sg",
            vpc=self.common_stack.custom_vpc,
            security_group_name=f"dms-{self.common_stack.deploy_env}-sg",
        )

        self.dms_subnet_group = dms.CfnReplicationSubnetGroup(
            scope,
            f"dms-{self.common_stack.deploy_env}-replication-subnet",
            ...
            replication_subnet_group_identifier=f"dms-{self.common_stack.deploy_env}-replication-subnet",
        )

        self.instance = dms.CfnReplicationInstance(
            scope,
            f"dms-replication-instance-{self.common_stack.deploy_env}",
            allocated_storage=100,
            publicly_accessible=False,
            engine_version="3.3.3",
            replication_instance_class="dms.t2.small",
            replication_instance_identifier=f"dms-{self.common_stack.deploy_env}-replication-instance",
        ...
        super().__init__(
            scope,
            f"{self.common_stack.deploy_env}-dms-task-orders-rds",
            migration_type="full-load-and-cdc",
            replication_task_identifier=f"{self.common_stack.deploy_env}-dms-task-orders-rds",
